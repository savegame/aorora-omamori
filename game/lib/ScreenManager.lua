...
local MusicVolume = class { __includes = chain.Chain,
	...
	onUpdate = function(self, dt)
		...
		local newVolume = math.max(0.0, math.min(1.0,
			self.initial + self.shift * (self.time / self.duration)))
  ...
  onInput = function(self, event) end
}
...
function ScreenManager:init(canvas)
	...
	self.fadeblack.x, self.fadeblack.y = 160, 120
	self.music = nil
...
function ScreenManager:changeMusic(newMusic, changeDuration)
	...
	c:push(chain.Rings()
		:pushParallel(
		MusicVolume(newMusic, 1.0, dur))
		:pushParallel(chain.Dynamic(function()
			if self.oldMusic then
				return MusicVolume(self.oldMusic, 0.0, dur)
			else
			return chain.Instant(function() end) end
		end))
	)
...
function ScreenManager:fadeOutMusic(duration)
	...
	local c = chain.Dynamic(function()
		...
		if self.oldMusic then
			return MusicVolume(self.oldMusic, 0.0, dur)
		else return chain.Instant(function() end) end
	end)
...
function ScreenManager:registerEvents()
	...
	function love.draw(...)
		...
		love.graphics.draw(self.canvas, 0, 0, 0, love.graphics.getWidth() / self.canvas:getWidth(), love.graphics.getHeight() / self.canvas:getHeight())
	end
